...
```
For players who were traded midseason or played for multiple teams this year, there are multiple entries for their statistics with different teams. Luka Doncic, for example, has three entries - one for his time with the Lakers, one for his time with the Mavericks, and a third for his statistics over the entire season. 
...
There are many more players listed as shooting guards compared to the other positions. This is not too surprising, considering that NBA players' positions are less defined than they used to be, so wings are often classified as shooting guards by default.
...
The data is the same as before, but it's sorted by the highest number first, then descends as the numbers decrease. This data is still not categorized by team, though, so it's difficult to analyze team-by-team trends.
...
Small forward seems the most interesting to me because that group has the highest standard deviation of all five positions, and the second-lowest average minutes. The disparity between the average number of minutes played by a small forward, and the higher or lower percentiles could be an interesting story angle.
