...
```{r}
nbalogs_2001 <- read_csv("~/GitHub/jour479x_fall_2025/presentations/2000-01nbadata.csv")

centerlogs_2001 <- nbalogs_2001 |> filter(Pos=="C")
```

```{r}
centeraverages_2001 <- centerlogs_2001|> rename("ThreePointAtt" = `3PA`) |> rename("ThreePointPct" = `3P%`) |> na.omit(centerlogs_2025) |>  summarise(
  AverageAssists = mean(AST),
  AverageRebounds = mean(TRB),
  Average3PTATT = mean(ThreePointAtt),
  Average3PTPCT = mean(ThreePointPct * 100)
